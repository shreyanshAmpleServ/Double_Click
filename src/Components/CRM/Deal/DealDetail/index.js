...
import { dealDetailFn } from "Services/CRM/Deal"
...
const DealDetail = () => {
  ...
  const { data } = useQuery(["dealDetail", deal_id], () => dealDetailFn({ deal_id }), {
    enabled: Boolean(deal_id !== ":deal_id"),
  })
    ...
    <div className="flex flex-col gap-2">
      <CustomDiv className="!p-2">
        <div className="flex flex-col gap-1">
          <p className="font-semibold">{deal?.deal_no}</p>
          <p className="text-sm text-black text-opacity-70">{moment(deal?.created_date).format("lll")}</p>
        </div>
      </CustomDiv>
      <div className="flex gap-2">
        ...
        <CustomDiv className="flex !p-0 flex-col w-2/3">
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell isHead>Campaign Sub Type</TableCell>
                ...
                {campaign?.campaign_type_info?.map((influencer) => {
                  return (
                    <TableRow>
                      ...
                      <TableCell>{influencer?.campaign_subtype}</TableCell>
