...
import { workFromHomeApprovalFn } from "Services/HRM/LeaveApplication"
...
const ManageStatus = ({ isUpdatePermit, wfh_application_id, refetch, status }) => {
  ...
  const { mutate: workFromHomeApproval, isLoading } = useMutation(workFromHomeApprovalFn, {
    onSuccess: ({ data }) => {
  ...
  const handleSubmit = (application_status, reason = "") => {
    workFromHomeApproval({ wfh_application_id, application_status, reason })
  }
    ...
    <>
      ...
      <Popper placement="top" sx={{ zIndex: 1200 }} anchorEl={anchorEl} open={isOpen} className="pb-1">
        <Paper className="flex flex-col justify-center bg-white shadow-xl">
          <p className="px-3 text-lg text-center">Work From Home Approval</p>
          <Divider />
