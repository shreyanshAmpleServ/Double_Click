...
import { wfhApprovalAdminFn } from "Services/HRM/LeaveApplication"
import { useState } from "react"
import { useMutation } from "react-query"
import { toast } from "react-toastify"

const AdminApproval = ({ wfh_application_id, refetch, status }) => {
  const [anchorEl, setAnchorEl] = useState(null)
  const isOpen = Boolean(anchorEl)

  const { mutate: wfhApprovalAdmin } = useMutation(wfhApprovalAdminFn, {
    onSuccess: ({ data }) => {
  ...
  const handleSubmit = (approval_status) => wfhApprovalAdmin({ wfh_application_id, approval_status })
