...
const AboutSection = ({ data, customWidth, value, type, isMarked }) => {
  ...
  return (
    <>
      {type == 1 ? (
        <div
          style={{ fontSize: `${customWidth.fontSize && fontSizeCalc(customWidth.fontSize)}` }}
          className={`flex ${widthCalculate(value)} w-[100%] ${
            customWidth.padding ? customWidth.padding : "p-[1%]  pb-0"
        ...
        >
          <div className="flex flex-col items-start ">
           ...
           <span className="text-primary2">About </span> 
            ...
            <div>
              <div dangerouslySetInnerHTML={{ __html: customWidth.fontSize ? data.replace("font-size:", "") : data }} />
            </div>
            {/* <div className="text-base text-slate-600 mb-4">
            Energistically utilize team driven niche markets rather than leveraged platforms. Monotonectally restore
            tactical "outside the box" thinking and technically sound deliverables.
        ...
        <div className={`flex flex-col w-[100%] ${widthCalculate(value)}  p-[1%] pb-0 justify-center `}>
          {data?.mime?.includes("image") ? (
            <img
              src={data?.url ? baseURL + data?.url : mapImage}
              alt="Map"
              style={customWidth.innerStyleCSS}
              className={` ${customWidth.innerHtmlCSSClasses}  mx-auto h-full object-fill object-center`}
            />
          ) : (
            <div
            ...
            >
              <div
                className="relative mx-auto w-[80%] rounded-2xl  lg:h-[50vh]  h-[30vh]"
                style={{
