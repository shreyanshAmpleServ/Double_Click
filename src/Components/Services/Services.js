  ...
  return (
    <>
      <div className="bg-sky-blue-100 p-[6%] !pb-[4%]">
        ...
        {mainServiceData?.data?.data?.[0]?.details?.map((item)=>(  <div className="box-item group relative  !rounded-md  text-white shadow-md hover:shadow-2xl h-[40vh]  overflow-hidden">
            <div
            ...
            <div className="absolute z-30 top-[50%] h-[60vh] mx-auto w-full flex flex-col gap-3 items-center group-hover:-translate-y-4    transition-all duration-300  ">
              <div>
               ...
               {item?.icon?.url ?  <img src={baseURL+item?.icon?.url} className="scale-x-[-1] h-10 w-10" />
               :  <TbTruckDelivery className="!text-5xl text-white transform scale-x-[-1]" />}
              </div>
